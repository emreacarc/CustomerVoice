...
LANG_DICT = {
    "TR": {
        ...
        "info_box": "Şikayet Metinleri İçin Analiz, Churn Skor Hesaplama, Time Series Tahminleme, Duygu Analizi ve Özetleme Platformu",
        ...
        "tabs": ["📊 Dashboard", "🔍 Şikayet Analizi", "📈 Zaman Serisi", "📝 Özet", "ℹ️ Hakkında"],
    ...
    "EN": {
        ...
        "info_box": "Analysis, Churn Score Calculation, Time Series Forecasting, Sentiment Analysis and Summarization Platform for Complaint Texts",
        ...
        "tabs": ["📊 Dashboard", "🔍 Complaint Analysis", "📈 Time Series", "📝 Summary", "ℹ️ About"],
...
def load_summary_models():
    ...
    try:
        ...
        # Özetleme Modeli (mT5)
        ozet_tokenizer = AutoTokenizer.from_pretrained(ozet_path, local_files_only=True)
        ozet_model = AutoModelForSeq2SeqLM.from_pretrained(
            ozet_path, 
        ...
        summarizer = pipeline(
            "summarization",
            model=ozet_model,
            tokenizer=ozet_tokenizer,
            device=device_id
        )
        
        # Duygu Modeli (XLM-RoBERTa)
        duygu_tokenizer = AutoTokenizer.from_pretrained(duygu_path, local_files_only=True)
        duygu_model = AutoModelForSequenceClassification.from_pretrained(
            duygu_path, 
            local_files_only=True,
            torch_dtype=torch_dtype
        ).to("cuda" if torch.cuda.is_available() else "cpu")
        ...
        classifier = pipeline(
            ...
            model=duygu_model,
            tokenizer=duygu_tokenizer,
            device=device_id
        )
        
        return summarizer, classifier
    except Exception as e:
        st.error(f"⚠️ Yerel modeller yüklenirken hata oluştu: {str(e)}")
        return None, None
...
# =========================================================
@st.cache_data
def get_summary_and_sentiment(_summarizer, _classifier, full_text):
    """Özet ve duygu analizini önbelleğe alarak hızlandırır"""
    # Özet
    ozet_sonuc = _summarizer(
        full_text,
        num_beams=4,
        max_length=150,
        min_length=20,
    ...
    duygu_sonuc = _classifier(
        full_text,
        ETIKETLER,
        multi_label=False
    )
    return ozet_metni, duygu_sonuc
...
def show_summary_analysis():
    ...
    try:
        ...
        st.error(f"❌ {'Modeller yüklenemedi' if st.session_state.lang == 'TR' else 'Models could not be loaded'}: {str(e)}")
        return
    
    if summarizer is None or classifier is None:
        st.error("⚠️ " + ("Modeller bulunamadı." if st.session_state.lang == "TR" else "Models not found."))
        return
    
    col_left, col_right = st.columns([1, 1], gap="large")
    with col_left:
        baslik = st.text_input(t('baslik_label'), placeholder=t('baslik_placeholder'))
        sikayet_metni = st.text_area(t('metin_label'), height=350, placeholder=t('metin_placeholder'))
        analiz_butonu = st.button(t('start_analyze_btn'), type="primary", use_container_width=True)
    
    with col_right:
        if analiz_butonu and sikayet_metni.strip():
            full_text = f"{baslik} {sikayet_metni}" if baslik.strip() else sikayet_metni
            with st.spinner("🔄 " + ("Analiz ediliyor..." if st.session_state.lang == "TR" else "Analyzing...")):
                ozet_metni, duygu_sonuc = get_summary_and_sentiment(summarizer, classifier, full_text)
                
                # ŞİKAYET ÖZETİ BÖLÜMÜ
                st.markdown(f"{t('summary_label')}")
                st.markdown(f'<div style="background: rgba(99, 102, 241, 0.1); border-left: 4px solid #6366f1; border-radius: 8px; padding: 1.5rem; margin-bottom: 2rem;"><p style="font-size: 1.05rem; line-height: 1.6; color: #f1f5f9; margin: 0;">{ozet_metni}</p></div>', unsafe_allow_html=True)
                
                # DUYGU ANALİZİ BÖLÜMÜ
                st.markdown(f"{t('sentiment_label')}")
                duygu_labels = duygu_sonuc["labels"]
                duygu_scores = [s * 100 for s in duygu_sonuc["scores"]]
                
                # Çeviri yap
                if st.session_state.lang == "EN":
                    en_labels = {"Öfke": "Anger", "Hayal Kırıklığı": "Frustration", "Bıkkınlık": "Exhaustion", "Endişe": "Worry", "Şaşkınlık": "Surprise", "Tehdit": "Threat"}
                    duygu_labels = [en_labels.get(l, l) for l in duygu_labels]
                
                duygu_renkler = {"Öfke": "#ef4444", "Hayal Kırıklığı": "#f59e0b", "Bıkkınlık": "#8b5cf6", "Endişe": "#6366f1", "Şaşkınlık": "#10b981", "Tehdit": "#dc2626",
                                 "Anger": "#ef4444", "Frustration": "#f59e0b", "Exhaustion": "#8b5cf6", "Worry": "#6366f1", "Surprise": "#10b981", "Threat": "#dc2626"}
                
                fig = go.Figure(go.Bar(x=duygu_labels, y=duygu_scores, marker_color=[duygu_renkler.get(l, "#6366f1") for l in duygu_labels]))
                fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='#fff', height=300, margin=dict(l=20, r=20, t=20, b=40))
                st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("👈 " + ("Analizi başlatmak için metin girin." if st.session_state.lang == "TR" else "Enter text to start analysis."))
# =========================================================
# HAKKINDA SAYFASI
# =========================================================
...
def main():
    ...
    with tab4: show_summary_analysis()
    with tab5: show_about_section()
